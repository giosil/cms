...
public
class GUILingue extends AEntityEditor implements ILingua
{
  private static final long serialVersionUID = -1644286284627811961L;
      ...
      throws Exception
  {
    ...
    oRecords = WUtil.toListOfMapObject(oRPCClient.execute("LINGUE.find", parameters, true));
    ...
    if(oRecords.size() == 1) {
      ResourcesMgr.getStatusBar().setText("1 lingua trovata.");
    }
    else {
      ResourcesMgr.getStatusBar().setText(oRecords.size() + " lingue trovate.");
    }
  }
      ...
      throws Exception
  {
    ...
    List<Object> parameters = new ArrayList<Object>();
    parameters.add(oId);
    Map<String, Object> mapRead = WUtil.toMapObject(oRPCClient.execute("LINGUE.read", parameters));
      ...
      throws Exception
  {
    ...
    if(boNew) {
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("LINGUE.insert", parameters));
      ...
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("LINGUE.update", parameters));
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("LINGUE.delete", parameters), false);
    if(oResult == null || !oResult.booleanValue()) {
      GUIMessage.showWarning("Lingua non eliminabile.");
      return;
    }
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("LINGUE.setEnabled", parameters), oAttivo);
