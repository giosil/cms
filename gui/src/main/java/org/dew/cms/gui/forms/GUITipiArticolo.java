...
public
class GUITipiArticolo extends AEntityEditor implements ITipoArticolo
{
  private static final long serialVersionUID = -5605300906224164228L;
      ...
      throws Exception
  {
    ...
    oRecords = WUtil.toListOfMapObject(oRPCClient.execute("TIPOLOGIE.find", parameters, true));
    ...
    if(oRecords.size() == 1) {
      ResourcesMgr.getStatusBar().setText("1 tipo articolo trovato.");
    }
    else {
      ResourcesMgr.getStatusBar().setText(oRecords.size() + " tipi articolo trovati.");
    }
  }
      ...
      throws Exception
  {
    ...
    List<Object> parameters = new ArrayList<Object>();
    parameters.add(oId);
    Map<String, Object> mapRead = WUtil.toMapObject(oRPCClient.execute("TIPOLOGIE.read", parameters));
      ...
      throws Exception
  {
    ...
    if(boNew) {
      Boolean oExists = WUtil.toBooleanObj(oRPCClient.execute("TIPOLOGIE.exists", parameters), false);
      if(oExists != null && oExists.booleanValue()) {
        GUIMessage.showWarning("Codice tipologia gi\340 utilizzato.");
        return false;
      }
      
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("TIPOLOGIE.insert", parameters));
      ...
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("TIPOLOGIE.update", parameters));
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("TIPOLOGIE.delete", parameters), false);
    if(oResult == null || !oResult.booleanValue()) {
      GUIMessage.showWarning("Tipo articolo non eliminabile.");
      return;
    }
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("TIPOLOGIE.setEnabled", parameters), oAttivo);
