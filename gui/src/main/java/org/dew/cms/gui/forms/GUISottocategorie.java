...
public
class GUISottocategorie extends AEntityEditor implements ISottocategoria
{
  private static final long serialVersionUID = -1284996111880596402L;
  ...
  Container buildGUIFilter()
  {
    ...
    fp.addTab("Sottocategoria");
    fp.addRow();
    fp.addOptionsField(sID_CATEGORIA, "Categoria", Opzioni.getCategorie(false));
    fp.addRow();
      ...
      throws Exception
  {
    ...
    oRecords = WUtil.toListOfMapObject(oRPCClient.execute("SOTTOCATEGORIE.find", parameters, true));
    ...
    if(oRecords.size() == 1) {
      ResourcesMgr.getStatusBar().setText("1 sottocategoria trovata.");
    }
    else {
      ResourcesMgr.getStatusBar().setText(oRecords.size() + " sottocategoria trovate.");
    }
  }
      ...
      throws Exception
  {
    ...
    List<Object> parameters = new ArrayList<Object>();
    parameters.add(oId);
    Map<String, Object> mapRead = WUtil.toMapObject(oRPCClient.execute("SOTTOCATEGORIE.read", parameters));
      ...
      throws Exception
  {
    ...
    if(boNew) {
      Boolean oExists = WUtil.toBooleanObj(oRPCClient.execute("SOTTOCATEGORIE.exists", parameters), false);
      if(oExists != null && oExists.booleanValue()) {
        GUIMessage.showWarning("Codice sottocategoria gi\340 utilizzato.");
        return false;
      }
      
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("SOTTOCATEGORIE.insert", parameters));
      ...
      Map<String, Object> mapResult = WUtil.toMapObject(oRPCClient.execute("SOTTOCATEGORIE.update", parameters));
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("SOTTOCATEGORIE.delete", parameters), false);
    if(oResult == null || !oResult.booleanValue()) {
      GUIMessage.showWarning("Sottocategoria non eliminabile.");
      return;
    }
      ...
      throws Exception
  {
    ...
    Boolean oResult = WUtil.toBooleanObj(oRPCClient.execute("SOTTOCATEGORIE.setEnabled", parameters), oAttivo);
