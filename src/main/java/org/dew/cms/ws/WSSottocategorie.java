...
import org.dew.cms.common.ISottocategoria;
import org.dew.cms.util.CMSCache;
import org.dew.cms.util.ConnectionManager;
import org.dew.cms.util.QueryBuilder;

public 
class WSSottocategorie implements ISottocategoria 
{
  protected static Logger oLogger = Logger.getLogger(WSSottocategorie.class);
      ...
      throws Exception
  {
    ...
    qb.put("ID_SOTTOCATEGORIA", sID);
    qb.put("ID_CATEGORIA",      sID_CATEGORIA);
    qb.put("CODICE",            sCODICE + "%");
    String sAddClause = "ATTIVO='" + QueryBuilder.decodeBoolean(true) + "'";
    String sSQL = qb.select("CMS_SOTTOCATEGORIE", mapFilter, sAddClause);
    ...
    try {
      ...
      oLogger.error("Exception in WSSottocategorie.lookup(" + mapFilter + ")", ex);
      throw ex;
    }
    finally {
      ...
      throws Exception
  {
    ...
    String sAddClause = "ID_SOTTOCATEGORIA<>0";
    String sSQL = qb.select("CMS_SOTTOCATEGORIE", mapFilter, sAddClause);
    sSQL += " ORDER BY CODICE";
    ...
    try {
      ...
      oLogger.error("Exception in WSSottocategorie.find(" + mapFilter + ")", ex);
      throw ex;
    }
    finally {
      if(rs   != null) try{ rs.close();  } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    qb.add("CODICE");
    qb.add("ATTIVO");
    String sSQL = qb.select("CMS_SOTTOCATEGORIE");
    sSQL += "WHERE ID_SOTTOCATEGORIA=?";
    ...
    try {
      ...
      pstm = conn.prepareStatement("SELECT ID_LINGUA,DESCRIZIONE FROM CMS_SOTTOCATEGORIE_DESC WHERE ID_SOTTOCATEGORIA=? ORDER BY ID_LINGUA");
      pstm.setInt(1, iId);
      rs = pstm.executeQuery();
      while(rs.next()) {
      ...
      oLogger.error("Exception in WSSottocategorie.read(" + iId + ")", ex);
      throw ex;
    }
    finally {
      if(rs   != null) try{ rs.close();   } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    try {
      conn = ConnectionManager.getDefaultConnection();
      pstm = conn.prepareStatement("SELECT ID_SOTTOCATEGORIA FROM CMS_SOTTOCATEGORIE WHERE CODICE=?");
      pstm.setString(1, sCodice);
      rs   = pstm.executeQuery();
      return rs.next();
    }
    catch (Exception ex) {
      oLogger.error("Exception in WSSottocategorie.exists(" + mapValues + ")", ex);
      throw ex;
    }
    finally {
      if(rs   != null) try{ rs.close();   } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    try {
      ...
      oLogger.error("Exception in WSSottocategorie.getDescrizione(" + iId + "," + iIdLingua + ")", ex);
      throw ex;
    }
    finally {
      if(rs   != null) try{ rs.close();   } catch(Exception ex) {}
    ...
    if(result == null) result = sDescrizione0;
    return result;
  }
      ...
      throws Exception
  {
    ...
    qb.add("CODICE");
    qb.add("ATTIVO");
    String sSQL = qb.insert("CMS_SOTTOCATEGORIE", true);
    ...
    try {
      ...
      int iId = ConnectionManager.nextVal(conn, "SEQ_CMS_SOTTOCATEGORIE");
      ...
      CMSCache.mapDescSot.clear();
      
      mapValues.put(sID, iId);
    }
    catch (Exception ex) {
      if(ut != null) try{ ut.rollback(); } catch(Exception exut) {}
      oLogger.error("Exception in WSSottocategorie.insert(" + mapValues + ")", ex);
      throw ex;
    }
    finally {
      if(pstm != null) try{ pstm.close(); } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    qb.add("ID_CATEGORIA");
    qb.add("CODICE");
    String sSQL = qb.update("CMS_SOTTOCATEGORIE", true);
    sSQL += "WHERE ID_SOTTOCATEGORIA=?";
    ...
    try {
      ...
      if(ut != null) try{ ut.rollback(); } catch(Exception exut) {}
      oLogger.error("Exception in WSSottocategorie.update(" + mapValues + ")", ex);
      throw ex;
    }
    finally {
      if(pstm != null) try{ pstm.close(); } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    qb.add("ID_LINGUA");
    qb.add("DESCRIZIONE");
    String sSQL = qb.insert("CMS_SOTTOCATEGORIE_DESC", true);
      ...
      throws Exception
  {
    PreparedStatement pstm = null;
    try {
      pstm = conn.prepareStatement("DELETE FROM CMS_SOTTOCATEGORIE_DESC WHERE ID_SOTTOCATEGORIA=?");
      pstm.setInt(1, iId);
      pstm.executeUpdate();
    }
    finally {
      ...
      throws Exception
  {
    ...
    try {
      ...
      pstm = conn.prepareStatement("UPDATE CMS_SOTTOCATEGORIE SET ATTIVO=? WHERE ID_SOTTOCATEGORIA=?");
      ...
      if(ut != null) try{ ut.rollback(); } catch(Exception exut) {}
      oLogger.error("Exception in WSSottocategorie.setEnabled(" + iId + "," + boEnabled + ")", ex);
      throw ex;
    }
    finally {
      if(pstm != null) try{ pstm.close(); } catch(Exception ex) {}
      ...
      throws Exception
  {
    ...
    try {
      ...
      pstm = conn.prepareStatement("DELETE FROM CMS_SOTTOCATEGORIE WHERE ID_SOTTOCATEGORIA=?");
      pstm.setInt(1, iId);
      pstm.executeUpdate();
      ...
      if(ut != null) try{ ut.rollback(); } catch(Exception exut) {}
      oLogger.error("Exception in WSSottocategorie.delete(" + iId + ")", ex);
      return false;
    }
    finally {
      if(pstm != null) try{ pstm.close(); } catch(Exception ex) {}
